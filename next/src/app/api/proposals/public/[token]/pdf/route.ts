...
export async function GET(_req: Request, ctx: { params: { token: string } }) {
  const prop = await prisma.proposal.findUnique({ where: { publicToken: ctx.params.token }, include: { project: true, items: { orderBy: { createdAt: 'asc' } } } })
  ...
  doc.fontSize(18).text('Propunere (Public)', { align: 'right' })
  doc.moveDown(0.5)
