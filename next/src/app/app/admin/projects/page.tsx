...
export default async function AdminProjectsPage({ searchParams }: PageProps) {
  ...
  // Calculate stats
  const [totalProjects, activeProjects, completedProjects, pendingProjects] = await Promise.all([
    prisma.project.count(),
    prisma.project.count({ where: { status: 'in_progress' } }),
    prisma.project.count({ where: { status: 'done' } }),
    prisma.project.count({ where: { status: 'request' } })
  ])
  ...
  return (
    <AdminLayout
      ...
      subtitle="Vizualizare și gestionare toate proiectele"
      actions={
    ...
    >
        ...
        <Card className="group relative overflow-hidden hover:shadow-2xl transition-all duration-500 hover:-translate-y-3 border-0 bg-gradient-to-br from-blue-50 to-white dark:from-blue-900/20 dark:to-slate-900 shadow-xl">
          <div className="relative z-10">
            <CardHeader className="flex flex-row items-center justify-between space-y-0 pb-3">
              ...
              <div className="text-3xl font-black text-blue-600 dark:text-blue-400">{totalProjects}</div>
            </CardHeader>
            <CardContent>
        ...
        <CardContent className="p-6">
          <div className="flex flex-col lg:flex-row gap-4">
            <div className="flex-1 relative">
              ...
              <Input
                placeholder="Caută proiecte după nume sau client..."
      ...
      <Card className="border-0 shadow-xl bg-white/95 dark:bg-slate-900/95 backdrop-blur-xl">
        <CardHeader>
          ...
          <CardDescription>
            Toate proiectele din platformă ({totalCount})
          </CardDescription>
        </CardHeader>
        <CardContent>
